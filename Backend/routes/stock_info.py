...
from data_fetcher import get_ticker_info, get_spy_period, get_ohlcv, get_earnings_dates as cached_get_earnings_dates, clear_cache
...
def get_stock_info(ticker):
    try:
        ...
        logger.error(f'Error with {ticker}: {str(e)}', exc_info=True)
        return jsonify({'error': str(e)}), 500


@stock_info_bp.route('/api/stock-info/<ticker>/refresh', methods=['POST', 'OPTIONS'])
@cross_origin(supports_credentials=True)
def refresh_stock_info(ticker):
    """Clear cache and fetch fresh stock info data."""
